...
function loadHistory(data) {
    const container = document.getElementById('activity-history'); // ID из HTML может отличаться, проверим
    // В HTML у нас activity-history, в JS тоже. Но в HTML это <div id="activity-history"></div>
    if (!container) return;
    container.innerHTML = '';
    if (data.history.length === 0) { container.innerHTML = '<p>История пуста</p>'; return; }
    data.history.slice(0, 10).forEach(item => {
        const div = document.createElement('div');
        div.className = 'history-item';
        div.innerHTML = `<div class="history-date">${item.date}</div><div class="history-action">${item.action}</div>${item.score ? `<div class="history-score">Результат: ${item.score}%</div>` : ''}`;
...
function showAchievementPopup(achievement) {
    ...
    const descEl = document.getElementById('achievement-desc'); // Added for clarity
    ...
    setTimeout(() => {
        popup.classList.add('hide'); // Need CSS for .hide animation
...
function initTestProgress() {
    ...
    for (let i = 0; i < cyberQuestions.length; i++) {
        ...
        dot.className = "progress-dot"; dot.id = `test-dot-${i}`;
...
function loadQuestion() {
    ...
    question.answers.forEach((answer, index) => {
        ...
        answerElement.innerHTML = `<span class="answer-letter" style="display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50%;background:var(--neon-blue);color:white;margin-right:15px;font-weight:bold;">${String.fromCharCode(65 + index)}</span> ${answer}`;
...
function analyzePassword(password) {
    const reqs = { length: document.getElementById("length-req"), upper: document.getElementById("upper-req"), lower: document.getElementById("lower-req"), number: document.getElementById("number-req"), special: document.getElementById("special-req"), common: document.getElementById("common-req") };
...
function initFileScanner() {
    ...
    fileInput.addEventListener('change', (e) => { if(e.target.files.length) scanFile(e.target.files[0]); });
...
function completeScan(file) {
    ...
    if (Math.random() > 0.3) {
        res.className = "scan-result safe";
        document.getElementById("file-name").textContent = file.name;
    } else {
        res.className = "scan-result danger";
